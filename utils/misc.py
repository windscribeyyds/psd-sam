...
class MetricLogger(object):
	...
	def log_every(self, iterable, print_freq, header=None, logger=None, print_func=print):
...
def mask_to_boundary(mask, dilation_ratio=0.02, value=0):
	"""
	...
	new_mask = cv2.copyMakeBorder(mask, 1, 1, 1, 1, cv2.BORDER_CONSTANT, value=value)
...
def generalized_image_grad(image):
	im_arr = image.cpu().numpy().transpose((1,2,0)).astype(np.uint8)
